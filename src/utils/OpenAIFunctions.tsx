import { Question, Score, Topic } from "./types";
...
const GPTApiCall = async (
  ...
  // Making a POST request to the local API endpoint
...
async function checkCache(
  ...
  hashStringWithSHA1(promptArr.join("")).then(async (hashValue) => {
    try {
      const response = await fetch(
        "https://learnanything-uk.stackstaging.com/r/" + hashValue
      );
      if (!response.ok) {
        GPTApiCall(promptArr, chunkFunction, trained, stream); // cache response failed, call GPT API
      }
      ...
      try {
        ...
        if (data.substring(0, 6) == "[DONE]") {
          ...
          GPTApiCall(promptArr, chunkFunction, trained, stream); // cache response is invalid format, call GPT API
        }
      } catch (error) {
        GPTApiCall(promptArr, chunkFunction, trained, stream); // cache response is invalid format, call GPT API
      }
...
[title of the test]
  ...
  checkCache(promptArr, chunkFunction, trained, stream);
